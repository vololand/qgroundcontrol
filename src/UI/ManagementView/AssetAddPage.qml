 ...
 * 트리거:
 *   - 우측 패널 자산 탭 "신규 등록" 버튼
 *   - 좌측 트리 group·subGroup 노드 우클릭 → 자산 등록
 *
 ****************************************************************************/
...
Item {
    ...
    property var    groupModel:      []    // org·group·subGroup 목록
    property string presetGroupName: ""   // 우클릭 대상 그룹명 (pre-select용)
    ...
    readonly property real _lblW:   _fW * 9   // 라벨 공통 너비

    // groupModel이 주입된 뒤 presetGroupName으로 콤보 인덱스 설정
    function _applyPresetGroup() {
        if (_root.presetGroupName === "") {
            _groupCombo.currentIndex = 0
            return
        }
        for (var i = 0; i < _root.groupModel.length; i++) {
            if (_root.groupModel[i] === _root.presetGroupName) {
    ...
    onGroupModelChanged:      Qt.callLater(_applyPresetGroup)
    onPresetGroupNameChanged: Qt.callLater(_applyPresetGroup)
    ...
    Rectangle {
        ...
        Item {
            ...
            ColumnLayout {
                ...
                // ── 상단: 좌/우 반반 필드 (Item + horizontalCenter로 정확히 50%)
                Item {
                    Layout.fillWidth: true
                    height: Math.max(_leftTopCol.implicitHeight, _rightTopCol.implicitHeight)

                    readonly property real _gap: _fW * 1.25   // 컬럼 사이 간격 절반
        ...
        Item {
            ...
            QGCLabel {
                anchors.left:           parent.left
                anchors.verticalCenter: parent.verticalCenter
                text:                   qsTr("※ 선택하신 자산 타입에 따라 입력 항목이 변경됩니다.")
            ...
            Row {
                ...
                QGCButton {
                    ...
                    onClicked: {
                        ...
                        // [자산등록] 현재 서버 미연결 → 클라이언트 측에서 accepted 신호만 발송
                        // TODO: 서버 자산등록 API 호출 후 성공 시 accepted() 처리
                        _root.accepted()
                    }
                }
